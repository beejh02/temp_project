...
관리자 `DELETE /api/markets/{marketId}`는 미션 대상 시장도 삭제하고 204를 반환한다.
삭제 성공 후 관련 미션 카탈로그를 갱신하며, 대상 시장이 없으면 미션 목록을 빈 배열로
반환한다. 시장 삭제는 독립 점포 데이터나 익명 세션의 포인트·쿠폰을 삭제하지 않는다.
시장 등록 후에는 카탈로그를 다시 조회해 새 대상 ID를 사용한다. 세션 배정·보상 기록은
서버 실행 단위로 유지하며 시장 재등록으로 수령 기록을 초기화하지 않는다.
